...
`docs/commercial-gate.md`: `DEGRADED_BY_HOST_CPU` / `BLOCKED_BY_NO_ADMITTER`; Run 29 launched 2026-09-03 19:41 JST with everything above in place and was `BLOCKED_BY_NO_ADMITTER` — nobody in the room; Run 30 (20:29 JST) was the same on the native arm64 Attendee image — VM load 1.0–2.4 instead of 18–24, both bots knocking within 30 s — and again `BLOCKED_BY_NO_ADMITTER`; Run 31 is that configuration with a person in the room).
