...
| 119 | **The answer before the question, and what it costs to know today** (no meeting; `scripts/reality/gemini-conversation.ts` and raw-protocol probes, 20:25–20:55 JST, 07 Sep). Run 118's successor in the room began **14 answers in ten minutes and had 8 cut** with 4 transcripts: the provider delivered the user's final transcript with `turnComplete` — the end of the *model's* reply — so when a reply started the page had nothing to sanction it against. The words are now flushed when our own gate closes the user's turn, and the policy will adopt a provider's turn on the room having been *heard*, transcript or not. **Search grounding measured, not assumed**: `gemini-2.5-flash-native-audio-latest` with `googleSearch` answers with the day's actual Japanese news (grounded), while `gemini-3.1-flash-live-preview` **refuses the socket outright when the tool is declared** (1011 "exceeded your current quota"; the same key grounds on native audio, and REST grounding is 429). A tool that closes the session is worse than a question unanswered, so it is declared where it works and falls back on refusal. The cost of knowing today, same four recorded utterances: **3.1 first audio p50 922 ms / max 976 ms** against **2.5 native audio + search p50 3 251 ms / max 4 446 ms** — and 2.5 also answered 「はい、拝見しました」 about a document nobody shared and invented a schedule, both of which 3.1 declined honestly. 3.1 stays the conversation model; news needs grounding entitlement on the key or a search API of our own. Default voice changed from Kore ("Firm") to **Achird ("Friendly")** — 「現在の声はあまり好きじゃないです」 — with Leda and Sulafat as the two to compare |
| 118 | **Gemini one-to-one, the fixes in a real room — she answers, and the call does not howl** (`ENGINE=google`, `persona=friend_ja`, `proactivity=open`, `outbound=page`, room `uqb-`, 20:12–20:22 JST, 07 Sep; bot `bot_WP1yR7ZDVXRbxo7f`, admitted at 20:18). Page counters through the run: **transcripts 7 · answers begun 9 · spoken 121 frames · gate 8 opens / 8 closes / 0 forced**, and four answers reached the room out loud (5.6 s greeting, then 3.8 s, 4.4 s, 4.5 s — 「ほうれん草があるよ。あとホタテもおいしいよね。」). The turn that never ended is gone: the gate opened and closed once per utterance all run. **Three faults reported live and fixed between runs.** (1) 「すごくハウリングしています」 — the human harness sent her voice back over the relay *and* the webpage streamer captured the page's own speaker, so the room heard her twice, half a second apart, each copy feeding the next; the scripted harness had always used the page path, this one defaulted to the socket because that is the copy it could count. Default is now `page`. (2) 「共有していない資料の話をしてきます」 — every instruction she had was written around a meeting's material, so with none present she invented it; `settingFor` now gives a one-to-one its own instructions (「会議ではありません」, 「見ていないものを見たことにしない」) and the persona is `friend_ja`, not the meeting colleague. (3) 「ニュースを教えてくれません」 — by design: no search tool exists, and the rule tells her to say so. Real-time answers need the tool, which is the user's own P2. Also fixed: the evidence table read this harness's parallel copy of the relay (0 chunks) and reported FAIL for a run the character's own page shows working. `gemini-gate2.log` |
| 117 | **Gemini one-to-one, why she heard everything and answered nothing** (`ENGINE=google`, `proactivity=open`, room `uqb-`, ~18:20–18:45 JST, 07 Sep; bot `bot_UKx1Qlo7LLyRlOY9`). The page's counters read `heard 38 572 · forwarded 38 572 · transcripts 48 · spoke 0 · sanctioned false`, state flapping LISTENING↔OBSERVING and never once ADDRESSED: audio was reaching the model, the model was transcribing it, and no reply was ever spoken. **Two faults.** (1) The gate's absolute -45 dBFS fallback (added the same afternoon for the opposite failure) is below the noise floor of an AGC'd meeting stream, so the gate opened on the room's own hiss and never closed — `activityEnd` was never sent and a model that is never told the turn ended never answers. Proved offline against the live API: the raw protocol replies **1 284 ms** after `activityEnd`, and the provider driven with a wav over -40 dBFS room tone now opens once, closes once (`opens 1 · closes 1 · forced 0 · openMs 5 260`) and answers 「今日は特に決まったことはないかな…」. (2) A speech-to-speech provider endpoints the room itself and starts replying ~1 s after it stops, while the page's own timer waits 2.5 s for the pause — so even a reply that did arrive was cut as `unsanctioned`. Fixed in `50881cb`: the gate's bar is relative to the VAD's learnt floor, no turn stays open past 8 s, and in a one-to-one the provider's own turn is adopted (`acceptSelfTurn`) instead of cut. 667 tests, build PASS. **Six turns on the fixed path, no meeting** (`scripts/reality/gemini-conversation.ts`, the room's own recorded utterances over -40 dBFS room tone, paced in real time): **6 / 6 answered, first sound 682 · 411 · 1 232 · 1 342 · 970 · 161 ms after the recording ends** (p50 970, p95 1 342), the gate **6 opens / 6 closes / 0 forced**, and **25 % of the room's audio sent** — the VAD keeps three quarters of a meeting off the bill. Replies natural and in character (「うん、見たよ。すごく分かりやすくまとまってて、助かった！」). The quiet required before a turn closes was 700 ms and is now 250: the VAD's own 500 ms hangover is the first guard, and over the same four utterances the speech-end-to-first-sound p50 fell **3 299 → 2 940 ms**. **Not yet proven in a room** — the real-Meet gate is the next knock |
| 116 | **720p tried and reverted — 5 / 8 · 5 / 7 · 5 / 7 · 5 / 7 with the ears at 387 holes / 239 s** (run 115's stack with `RCAI_STREAMER_FRAME_SIZE=1280x720`, everything else equal; 11:52–12:12 JST, 07 Sep; both bots admitted in 253 s). The character's tile at four times the pixels, rendered by software GL and VP8-encoded in Python: **143 ears holes in pass 1 alone against run 115's 45**, 387 / 238.8 s over the admission (run 115: 182 / 110.6 s), bot host 491–704 % CPU, host load 15, and 12 504 zero chunks — `ask1` and `followup` lost in passes 2–4 before they reached her, one `bargein` with no `interrupted` at all. `page render rate` stayed PASS (53 fps at the page) and the replies that did get out kept the 48 kHz band (f99 up to 6 969 Hz): the picture was not the thing that broke, the machine was. **Reverted to 640x360** the moment the run ended (`attendee-selfhost.sh up` with the default). 720p on this host costs the character's ears; it belongs to a bot host with a hardware encoder. `auto-run116.log` |
| 115 | **One admission, four passes — 7 / 8 · 5 / 7 · 5 / 7 · 6 / 7; the 48 kHz voice again, the room open all run** (run 90's launch line + `SETTLE_MS`; 11:20–11:42 JST, 07 Sep; both bots admitted in 352 s, pre-check 2 111 chunks; host load 5–11, down from run 114's 50). **f99 across the run: median 3 875 Hz, up to 7 156 Hz** (16 kHz capture had held 1.6–2.5 kHz since run 89) — the 48 kHz change is confirmed twice over. First token 238–1 681 ms (median 540). The room was open for the whole admission (**89 silent chunks in 105 381**, 0.08 %) and the recogniser fed her the room's own words: 「明日式」「サブペリメン」 reached two replies, the ears 182 holes / 110.6 s, and both cut-in cues of passes 2–3 never arrived (`bargein` PARTIAL ×4 — no `interrupted` to observe in any of them). `ask2` FAIL in pass 2 and PARTIAL `ask1` in pass 3 the same way: the question did not reach her. What did reach her was answered, including the demonstrative check the prompt asks for (「「それ」というのが、3ページ目の…のことでしょうか？」). Vendor transcript FAIL: Yui's `transcription_state` ended `failed` again. Reports in `rcai-auto-1788747330878`; `auto-run115.log`, agent `agent-run113.log` |
| 114 | **One admission, four passes — 6 / 8 · 6 / 7 · 6 / 7 · 5 / 7; the host at load 50, and the vendor API timed out mid-pass** (run 90's launch line + `SETTLE_MS`; 10:49–11:11 JST, 07 Sep; both bots admitted in 80 s, pre-check 1 262 chunks). The room was open again (59 silent chunks in 89 672) and the machine was in a storm: **ears 250 holes / 254 s of a 14-minute admission**, bot host 638–**1 232 %** CPU, host load average **50**, and in pass 4 `POST output_audio` hit `TimeoutError` — the harness's new `BLOCKED_BY_BOT_HOST` line fired and the pass carried on (d335e93 doing its job). What the host was doing, read from `ps` right after the run: `mobileassetd`, `ANECompilerService`, `modelcatalogd`, `mediaanalysisd`, `assistant_service`, `BackgroundShortcutRunner` — macOS's own asset download, Neural-Engine model compilation and media indexing, none of it ours (the Attendee containers were at 0.3–10 % CPU when idle). The character's own numbers held anyway: first token 453–859 ms (median 537), every question answered where it arrived, `bargein` PASS ×1 / PARTIAL ×2 / FAIL ×1 (the reply had ended or the cut-in never reached her), vendor transcript PASS. Replies stretched ×1.4–3.0 by underruns and f99 938–4 188 Hz — the 48 kHz gain is there in the clean ones (3.1–4.2 kHz) and buried in the starved ones. The rescore gate skipped 20 of 103. Reports in `rcai-auto-1788745768175`; `auto-run114.log`, agent `agent-run113.log` |
| 113 | **One admission, four passes — 6 / 8 · 7 / 7 · 6 / 7 · 5 / 7; the voice at 48 kHz heard, the host and the room still against her** (run 90's launch line + `SETTLE_MS`, streamer at 48 kHz, agent on the rescore gate and the clock line; 09:42–10:09 JST, 07 Sep; both bots admitted in 378 s, pre-check 2 496 chunks). **The voice:** f99 **6 375 / 4 125 / 3 969 / 6 313 / 3 844 / 6 719 Hz** on the replies the Tester heard cleanly — 16 kHz capture had held every run since 89 at 1.6–2.5 kHz; the remaining 2.1–2.2 kHz readings sit on replies with 0.6–1.5 s gaps (underruns, the host). Meet's captions read the greeting 「こんにちは。ゆいです。ゆい と声をかけてもらえれば…」. **The room open again** (54 silent chunks in 111 064 — 0.05 %; 0.4–4.0 s of sound in the quiet cues): pass 1 `third` FAIL on a turn the streaming recogniser earned with 「ユイがどう言ってたよね？」 (「昨日そう」 gone; name + question, and the third-person marker with it) — the rescore that would have withdrawn it took 8.6–14.8 s under load; a sound-alike turn on `ask1`; `ask2` answered about 「その穴日」 (the cut-in's 「話」 as the recogniser wrote it — the bearings line kept the opinion off the interruption but not the garbled word). **The host:** ears 143 holes / 67.7 s (34 / 23 / 45 / 43 a pass), 146 arrival holes, bot host 438–608 % CPU peaks, swap 14.9 GB, the voice over a second ×20, first token 375–1 543 ms (median 525 ms), the fold 8.5–15.1 s — pass 4 lost `ask1` and `followup` to the holes. The rescore gate skipped 12 of 75 second readings (chatter statements); the rest were questions or short. `bargein` PASS ×2, PARTIAL ×2 by timing. Reports in `rcai-auto-1788741718472`; `auto-run113.log`, agent `agent-run113.log` |
| 112 | **VOID as a check — 7 / 8 · 4 / 7 · 4 / 7 · 4 / 7 on a starved host: 365 ears holes / 188 s** (run 90's launch line + `SETTLE_MS`, the page rebuilt with the fragment hold and the bearings line; 08:49–09:26 JST, 07 Sep; Yui admitted in 30 s, the Tester 929 s later — 15 minutes at the door). The pre-check passed (79 904 chunks) and pass 1 went 7 / 8 (`bargein` PARTIAL by timing; `ask1` / `followup` / `ask2` answered through 2.6 s gaps), then the ears gave out: 64 holes a pass, 235 → 295 → 365 cumulative, 188 s of the 20-minute admission inaudible to the character, every question of passes 2–4 lost before it reached her (`turn=none`), one stray sound-alike turn in pass 2. The room was muted (85 % silent chunks); the host was swapping 14–15 GB with load 5–8 from other sessions. Nothing in this run can be read for or against the two changes it carried. Reports in `rcai-auto-1788738595096`; `auto-run112.log`, agent `agent-run102-111.log` |
| 111 | **One admission, four passes — 8 / 8 · 7 / 7 · 6 / 7 · 3 / 7; the microphone muted, the ears collapsed in pass 4** (run 90's launch line + `SETTLE_MS`, overlay stack; 00:58–01:19 JST, 07 Sep; both bots admitted in 48 s, pre-check 2 714 chunks). Passes 1–2 clean (`bargein` PASS ×2, greeting PASS, every question answered), pass 3 lost `ask2` and pass 4 lost `ask1` / `followup` / `ask2` with `bargein` PARTIAL by timing — all to the ears: **139 holes / 66.9 s over the admission, 54 of them / 52 s in pass 4 alone** (run 110: 34 / 14.7 s), the recogniser handed fragments 「あげよ、サメージメラ」「どの予定を教えて」「って週までに終わりそう？」 — no name survived, no turn was owed. The room was muted throughout (82 % silent chunks, 0.0 s of sound in every quiet cue): `DEGRADED_BY_HOST_CPU` alone — bot host 517–595 % CPU peaks, the voice over a second ×15, a 4.5 s re-warm, host swap 13.3 GB. Agent side: first token 300–1383 ms over 17 turns (median 448 ms), two folds re-warmed, no eviction, vendor transcript PASS. The character's path held where the sound reached it; from pass 3 on, most of it did not. Reports in `rcai-auto-1788710279627`; `auto-run111.log`, agent `agent-run102.log` |
| 110 | **One admission, four passes — 8 / 8 · 7 / 7 · 6 / 7 · 6 / 7 with the microphone muted; every FAIL-free, the two PARTIALs by timing** (run 90's launch line + `SETTLE_MS`, overlay stack; 00:30–00:51 JST, 07 Sep; both bots admitted in 44 s, pre-check 2 189 chunks). The room muted again (82 % silent chunks, 0.0 s of sound in every quiet cue) and the difference shows: no stray turn, no cut reply from the room, `ask1` / `followup` / `ask2` ×4, `third` / `chat` / `silence` quiet ×4, `bargein` PASS ×2 and PARTIAL ×2 by timing (passes 3–4: 3.5–5.5 s replies had ended before the cut-in's transcript arrived — nothing to stop, `interrupted` none). Greeting first token 220 ms and read by Meet's captions as 「ゆいです」; **first token 220–639 ms over 21 turns (median 369 ms)**, three folds (8.2 / 10.3 / 10.7 s) re-warmed (2.1–2.3 s), no eviction, vendor transcript PASS 21 / 60. Host: bot host 285–416 % CPU peaks, ears 34 holes / 14.7 s, (broker → page) 36 arrival holes, the voice over a second ×4, one reply stretched ×1.48 (underruns), swap 12.7 GB. Content: `ask2` after the barge-in answered 「まずは相手の方の意図をしっかり聞くのが大事」 ×3 — the cut-in's request still colouring the next answer (open, a prompt matter). Reports in `rcai-auto-1788708592104`; `auto-run110.log`, agent `agent-run102.log` |
| 109 | **One admission, four passes — 7 / 8 · 7 / 7 · 6 / 7 · 7 / 7; 27 of 29, the room open, a garbled name answered** (run 90's launch line + `SETTLE_MS`, overlay stack; 23:52–00:14 JST, 06→07 Sep; both bots admitted in 57 s, pre-check 1 845 chunks). Greeting first token 852 ms (the session's warm ran beside it) and read by Meet's captions as 「こんにちは。ゆいです。ゆい と声をかけていただければ…」; `bargein` PASS ×3 (cut-in +7.5–8.0 s → `interrupted` +10.1–11.9 s, 0.0 s of her own sound after) and PARTIAL by timing in pass 3; `ask1` / `followup` / `ask2` ×4; first token 274–434 ms on every turn after the greeting, three folds re-warmed (1.7–3.8 s), no eviction, vendor transcript PASS 23 / 63. **The one FAIL, pass 1 `third`:** 「ゆいが昨日そう言ってたよね」 reached the streaming recogniser as 「いいが、昨日そう言ってたよね。」 — the name gone into a hole — and with the policy still engaged from `followup`, a nameless line from the same speaker is an *engaged follow-up*: a turn, and an answer (first token 1.1 s after the text). The rescore's 「ゆいが昨日そう言ってたよね」 (third person) came 2.6 s after the streamed text, too late for the second opinion to withdraw a reply already speaking — run 78 pass 3's class, which the withdraw fixed only when the rescore arrives before audio. The next candidate: hold an engaged follow-up whose streamed text opens with a fragment (「、」 or a one-to-two-mora word before 「、」) for the rescore, up to ~2.5 s. The room was open all run (2 % silent chunks) with 0.1–2.9 s of sound in every quiet cue; the bot host at 458–567 % CPU peaks, ears 59 holes / 21.9 s, (broker → page) 88 arrival holes, the voice over a second ×11, three replies stretched ×1.24–1.69 by underruns; host swap 12–13 GB. Reports in `rcai-auto-1788706362425`; `auto-run109.log`, agent `agent-run102.log` |
| 108 | **`BLOCKED_BY_NO_ADMITTER` — three knocks, none admitted in the 1800 s budget** (23:04–23:38 JST, 06 Sep). Each knock died at Meet's limit (`fatal_error`), park mode restarted the worker between them (29–30 leaked Chromium per expired knock, as in runs 85–86); the harness's Tester relay client logged `401` / `500` from the broker while re-knocking — the relay token belongs to the first knock's bots and the re-knock does not refresh the harness's client (cosmetic during the wait; a row for the next harness fix). No bot in the room afterwards; worker at 2 Xvfb · 0 Chromium. `auto-run108.log` |
| 107 | **One admission, four passes — 8 / 8 · 7 / 7 · 6 / 7 · 7 / 7; 28 of 29 cues, with the room still open** (run 90's launch line + `SETTLE_MS`, overlay stack, worker restarted before the knock — 3 Xvfb · 14 Chromium from run 106; 22:38–22:59 JST, 06 Sep; both bots admitted in 45 s, pre-check 2 675 chunks). Greeting first token 221 ms and read by Meet's captions as 「私はゆいです。ゆい と声をかけて…」; `bargein` PASS ×4 (cut-in +7.4–8.0 s → `interrupted` +10.6–10.9 s, 0.0 s of her own sound after); `ask1` / `followup` / `ask2` ×4 but one; first token 221–493 ms on every turn, three folds re-warmed, no eviction, vendor transcript PASS 20 / 67. **The one FAIL, pass 3 `ask1`:** the room's own talk (7 % silent chunks all run — the microphone was open again, with 「ただのバラエティー番組」「緑になっていた」「全滅カード」 in it) had said 「ゆう今どう思う?」 seconds before the cue and taken a turn; the cue then reached the recogniser as 「イ、今日の予定を教えて。」 (the 「ゆ」 in a hole) while she was answering the room, and the rescore's 「ゆい、今日の予定を教えて」 came 3 s later — no turn for the question. Two more barge-ins were the room's, not the Tester's (13:45:43 Z at the start of a reply, 13:57:54 Z after `ask2`). Host: bot host 655 / 472 / 245 / 821 % CPU peaks, ears 43 holes / 17.6 s, (broker → page) 307 arrival holes, swap 12–14 GB, the voice at 1.0–2.1 s a phrase ×8. `DEGRADED_BY_OPEN_MIC` and `DEGRADED_BY_HOST_CPU` at once, and still 28 of 29. Reports in `rcai-auto-1788701885130`; `auto-run107.log`, agent `agent-run102.log` |
| 106 | **One admission, four passes — 5 / 8 · 6 / 7 · 7 / 7 · 7 / 7; the stack fixed, the room open again** (run 90's launch line + `SETTLE_MS`, Attendee recreated through the overlay; 22:06–22:27 JST, 06 Sep; both bots admitted in 427 s). **The pre-check passed with 2 534 chunks on the relay** — the overlay's `--disable-features=LocalNetworkAccessChecks` was the missing piece (runs 104–105) — and Meet's captions read the greeting as 「こんにちは。ゆいです。ゆい と声をかけて…」: the name reading (6dca32d) heard in the room. First token 247–584 ms on every turn, three folds re-warmed, no eviction, vendor transcript PASS 19 / 66. **`DEGRADED_BY_OPEN_MIC` again, with a television in it:** 5 % silent chunks (6 212 of 116 141; a muted room reads 80 %), the recogniser wrote 「続きましてが」「にやるって書いてる」 between the cues, the character's context filled with 「バンバー」「集計会」「オリンピック」 and two answers were about them; pass 1 `silence` FAIL (a turn taken on the room's own talk), `ask2` FAIL and `bargein` FAIL (her reply was cut by the room's sound before the cue's cut-in; `interrupted` none), pass 2 `followup` lost under a 7.2 s gap. The bot host went to **501 / 583 / 881 % CPU** in passes 2–4 with the ears at 27 / 64 / 89 holes (10.3 → 39.6 s cumulative) and 325 arrival holes — and the host swapped 13.8 GB while several other Claude Code sessions shared the machine; the voice at 1.3–1.9 s a phrase ×6. Everything the rig owns is back; what the room and the host add is outside it. Reports in `rcai-auto-1788699588934`; `auto-run106.log`, agent `agent-run102.log` |
| 105 | **VOID by the pre-check — `BLOCKED_BY_BOT_AUDIO` 30 s after admission, and the cause found** (21:22 JST, 06 Sep; both bots admitted in 135 s; the harness left on its own, no cue played). The rebuilt stack had been recreated from Attendee's plain `dev.docker-compose.yaml` — **without the project's overlay** `scripts/reality/attendee-selfhost/compose.rcai.yaml`, which is how the stack had always been started: it gives the worker `RCAI_EXTRA_CHROME_ARGS=… --disable-features=LocalNetworkAccessChecks` (Chromium 151 gates the Meet page's `ws://localhost` media socket — the one the bot's captured audio travels on — behind a Local Network Access prompt; the row-45 / row-181 finding), `RCAI_DISABLE_DEBUG_RECORDING=true` (no second x264 encoder per bot) and the 640x360 streamer on the arm64 image. Without the flag both bots' pages could not deliver a chunk to the worker, so the worker never opened its websocket to the broker: the 0-chunk relays of runs 104 and 105, the worker's `opening handshake failed … EOF` from the page. Ruled out on the way, in throwaway containers on the new VM: pulseaudio (playback and record stream), Chromium's WebAudio → `MediaStreamTrackProcessor` path and `getUserMedia` (headed on Xvfb: 20 frames, `gum-ok`; the headless variants never resolve and are not evidence), seccomp, `/dev/shm`, realtime priority. Recreated through `attendee-selfhost.sh up` with `RCAI_ATTENDEE_IMAGE=attendee-attendee-app-local:arm64`; the worker now carries the RCAI env. `auto-run105.log` |
| 104 | **VOID — `turn=none` ×16 on a bot host rebuilt after the prune; no audio ever reached either relay** (run 90's launch line; 17:48–18:00 JST, 06 Sep; both bots admitted in 81 s, stopped after pass 2). The page activated (`voice agent page started` PASS), the streamer loaded it and completed its WebRTC, the Tester's cue playback ran (`currently_playing_audio_media_request_finished` ×7) — and **neither bot's mixed-audio websocket ever connected to the broker** (no `relay connected` for either bot; `tester heard the room` 0 chunks; the worker's per-participant audio manager counted 0 chunks). Attendee starts that client on the first audio chunk from the bot's own Chromium, and no chunk came: the page's audio hook in Chromium 151 never delivered, while the worker's local websocket server logged `opening handshake failed … connection closed while reading HTTP request line` from its page. What had changed since run 103 (17:31, four clean passes): Docker Desktop restarted itself at 17:42, a `docker system prune -f` removed the three stopped Attendee containers, and app / scheduler / worker were recreated from the same compose file and the same image (source bind-mounted, config hash from the same file). Env, mounts, host config, Chrome policy file (`{}`), `/etc/hosts`, pulseaudio — all as before; the difference is not found. The whole stack was then recreated with `compose down` / `up --no-build` (streamer included) for the next knock, which is a probe. The harness now refuses to run cues against a deaf room: `BLOCKED_BY_BOT_AUDIO` if the character's relay carries no chunk within 30 s of the settle (d335e93). `auto-run104.log` |
| 103 | **One admission, four passes — 8 / 8 · 7 / 7 · 6 / 7 · 6 / 7; the corrected verdict, the character clean again** (run 90's launch line + `SETTLE_MS`, VM 8 GiB, harness on the afterCut verdict 33bd754; 17:10–17:31 JST, 06 Sep; Yui admitted in 30 s, the Tester 24 s later). `bargein` **PASS** in passes 1–2 on the character's own sound after the cut-in (0.0 s; the tail scan read 6.3 / 6.7 s and stays in the detail), PARTIAL by timing in passes 3–4 (the reply had ended before the cut-in's transcript arrived — no `interrupted` to observe; the reply's length decides this, 4.6–5 s replies end first). First token 228–500 ms on every turn but one (936 ms, a yield turn overlapping a re-warm), no voice phrase over a second, three folds re-warmed, no eviction, greeting first token 228 ms, ears 1–4 holes a pass, vendor transcript PASS. **A content wobble worth a row:** after the barge-in, `ask2` (「ゆい、今どう思う？」) was answered ×4 with variations of 「相手の方が先に話されるように、今は一旦待っているのが良いと思います」 — the cut-in's own request (「こっちの話を先にさせて」) carried into the next answer's content; coherent as a conversation, off the question's intent (the document). **The host during the run:** disk 6.8 → **5.1 GiB** in pass 4, then 18 GiB the moment the harness left — the run holds ≈ 13 GB of transient space (the bot host's recordings and browser caches inside the VM's image, trimmed after the bots' post-processing); a knock needs ≈ 15 GiB free, not 8. Docker Desktop then restarted itself at 17:42 (backend log: login reloaded, API proxy restarted — no I/O error this time) and a `docker system prune -f` run in that minute deleted the three stopped Attendee containers; recreated with `docker compose … up -d --no-build` after tagging the arm64 image with the service names compose expects. Reports in `rcai-auto-1788682206710`; `auto-run103.log`, agent `agent-run102.log` |
| 102 | **One admission, four passes — 7 / 8 · 6 / 7 · 6 / 7 · 6 / 7 on the 8 GiB VM; the character clean, the harness's verdict wrong** (run 90's launch line + `SETTLE_MS`, bot-host VM at 8 GiB, agent on the name reading 6dca32d, harness on the exclusion lag 150c905; 15:58–16:20 JST, 06 Sep; Yui admitted in 30 s, the Tester 18 s later). **The reply path: first token 210–450 ms on every turn, not one voice phrase over a second, ears (vendor → broker) PASS** (3 holes / 1.0 s in 19 minutes), greeting first token 210 ms, three folds re-warmed (0.8–2.1 s), no eviction; `ask1` / `followup` / `ask2` ×4, `third` / `chat` / `silence` quiet, vendor transcript PASS 21 / 57 with the greeting read 「こんにちは。ゆうです。ゆい と声をかけて…」 (the voice said ゆい twice — the second heard right, Meet's captions still uncertain on the first). **Every `bargein` PARTIAL, and none of it the character's:** passes 1–3 read `quiet 7.0 / 5.9 / 6.8 s after the interrupted` while `audible 1.5–5 s after the cut-in` was **0.0 s** in all three — the exclusion lag (150c905) took the Tester's cut-in out of the tail scan, and the 1.5 s of quiet the scan wants is broken by the yield 「はい。」 and its own gaps; pass 4 was timing (the reply had ended before the cut-in's transcript arrived, no `interrupted` to observe). The verdict is now the character's own sound after the cut-in (`AFTER_CUT_S` 1.0; 33bd754) — on that measure every observed barge-in of runs 90–102 passed. Host: swap climbed again to 9.6 GB and the disk to 6.8 GiB during the run even with the smaller VM — the host's other tenants, not the rig's. Reports in `rcai-auto-1788677923528`; `auto-run102.log`, agent `agent-run102.log` |
| 101 | **One admission, four passes — 7 / 8 · 7 / 7 · 7 / 7 · 7 / 7; the microphone closed, and a guard withdrawn** (run 90's launch line + `SETTLE_MS`, agent on the katakana rescore guard b0bee3b; 14:07–14:28 JST, 06 Sep; Yui admitted in 30 s, the Tester 25 s later). With the room's microphone muted the mix was 81 % silence again (26 224 of 32 132 chunks in pass 1), the greeting spoke in full (first token 210 ms, `spoke=105f`), `ask1` / `followup` / `ask2` answered ×4, `bargein` PASS ×3 and **PARTIAL in pass 1 on 6.2 s of sound after the `interrupted`** — Yui said 「はい。」 (0.5 s), the microphone was closed, so the admitter's mic was *not* the source: the same ≈ 6.2–6.4 s appears in runs 90, 98, 100 and 101, always once per admission, and is now an artefact to trace on the Tester's capture, not in the room. First token 210–466 ms on every turn, post-barge-in yields 388 / 455 / 456 / 466 ms, three folds re-warmed, no eviction. **The new guard did harm:** it dropped the *correct* rescore four times — the streaming recogniser writes 「ページ」 as 「定理」「定ジ」「経リ」 and whisper's 「3ページ目」 is what the room said; with the correction dropped the character herself said 「三定理目」 in three cut replies. Reverted (076e9da): a katakana word the first pass never heard is as often the right reading of garbled kanji as a name nobody said; the name hallucination stays open (vocabulary-aware drop — a run that has appeared nowhere in the session — is the next candidate, with the first-mention caveat). Meet's captions read the greeting's 「Yuiです」 as 「イです。ユーと声をかけて…」 (run 98: 「ゆうです」) — the Latin name is read unreliably by the voice; a kana reading for the name at synthesis is the fix to make. Supertonic 1.2–1.6 s ×3; muffled f99 1406–2594 Hz on seven replies; ears (vendor → broker) DEGRADED 21 holes / 12.1 s — 20 of them in pass 3 alone (the host: swap 12.4 of 13 GB, **disk 9.1 → 4.1 GiB across the run**, the swap files taking it). Vendor transcript PASS 19 / 59. Reports in `rcai-auto-1788671226959`; `auto-run101.log`, agent `agent-run101-done.log` |
| 100 | **One admission, four passes — 8 / 8 · 7 / 7 · 5 / 7 · 6 / 7; the reply path clean, the room's microphone open** (run 90's launch line + `SETTLE_MS`, agent on the barge-in fold cancel 6d0bcd8; 11:45–12:07 JST, 06 Sep; Yui admitted in 30 s, the Tester 113 s later). **First token 279–627 ms on every turn**, the post-barge-in yields 399 / 413 / 447 / 443 ms with a fold in flight at one of them (cancelled by the interrupt, folded after the next reply — the last coupling gone), three folds re-warmed before the next turn, no eviction. **`DEGRADED_BY_OPEN_MIC`:** the vendor's mix carried sound the whole admission — **1 all-zero chunk in 122 717** (runs 94–99: 80 % zeros), 0.2–2.0 s audible in every quiet cue, the greeting cut off by a barge-in 1.3 s after its first token (`spoke=0f`), 「うんうん。」 and garble (「でほど？」「よらでた。」) between the cues. Pass 3 `ask1` never reached the recogniser under it (no transcript at the cue's time) and `followup` had nothing to follow; pass 4 `bargein` PARTIAL on 6.2 s of that sound after the `interrupted` (Yui said 「はい。」). **The rescore's name again:** 「ユイが昨日そう言ってたよね」 → whisper 「ゆいがリノースを言ってたよね」, accepted (long clip, high overlap), and the next cut reply opened 「リノースさんが言っていた内容について…」. A guard for it (b0bee3b: drop a rescore that agrees on the sentence but adds a katakana run the first pass never heard) was tried and **withdrawn in run 101** (076e9da) — it dropped the correct 「3ページ目」 four times. Supertonic 1.0–3.0 s ×6 at the join and under the mic; muffled f99 1938–2281 Hz; gaps 1.1–3.1 s on five replies (the mic's sound inside the Tester's capture as much as the host); ears (vendor → broker) DEGRADED 14 holes / 4.6 s, (broker → page) **PASS**; vendor transcript PASS 17 / 58. Host: disk 8.7 → 9.3 GiB, swap 9.6 / 10 GB, Docker VM still 12 GB. Reports in `rcai-auto-1788662691188`; `auto-run100.log`, agent `agent-run100-done.log` |
| 99 | **One admission, four passes — 8 / 8 · 7 / 7 · 7 / 7 · 7 / 7, every cue PASS; the greeting warm** (run 90's launch line + `SETTLE_MS`, agent on the keep-warm dc69ee3; 10:06–10:28 JST, 06 Sep; Yui admitted in 30 s, the Tester 76 s later). **Greeting first token 561 ms** (3457 ms in run 98 — the 45 s keep-warm tick kept the weights resident through the admission wait; the pre-flight answered in 1.2 s where it had taken 5.8–8.2 s). `bargein` PASS ×4 (cut-in +6.9–8.2 s → `interrupted` +9.3–11.2 s, 0.0 s audible after — no stray sound this time), `greet` PASS at T0−25.1 s, no name echo. First token 262–451 ms on every turn but one: pass 2's post-barge-in yield at **3739 ms** — the fold behind the cut-off reply finished 1.2 s *after* the barge-in, its re-warm could not run (the yield turn owned the model), and that turn re-read **2403 tokens in 3.4 s** (the prompt from the notes on, the window at its 1.5-budget cap). Fixed: `interrupt()` now cancels a running fold, which picks up after the next reply (6d0bcd8, session test). The other three folds (10.3 / 10.5 / 10.9 s) re-warmed (3.4 / 1.3 / 1.3 s) before the next turn; no eviction. Supertonic 1.0–1.6 s ×10 (run 98: none) — the host at load 3 with swap at 9.1 of 10 GB; muffled f99 2094–2406 Hz on four replies; ears (vendor → broker) DEGRADED 15 holes / 5.1 s, (broker → page) 19 arrival holes; vendor transcript FAIL 0 / 0 (Attendee's transcriber). **Host disk 11 → 9.8 GiB during the run: the swap files** (10 GB allocated) — the 12 GB bot-host VM on a 16 GB host is now eating disk as well as time. Reports in `rcai-auto-1788656789599`; `auto-run99.log`, agent `agent-run99-done.log` |
| 98 | **One admission, four passes — 8 / 8 · 6 / 7 · 7 / 7 · 7 / 7; the cleanest run so far, ears PASS** (run 90's launch line + `SETTLE_MS` 20 s, worker restarted before the knock — 3 Xvfb · 14 Chromium left from run 97; 02:03–02:34 JST, 06 Sep; Yui admitted in 30 s, the Tester 637 s later). **Every first token 253–470 ms, every pass** — pass 1 `ask1` at 263 ms where run 97 paid 10.2 s (the 20 s settle after the Tester's admission did it); no Supertonic phrase over 1 s in the whole run (first time); three folds (6.5 / 6.3 / 8.1 s) each re-warmed (1.6 / 3.2 / 1.5 s) before the next turn; post-barge-in turns 407 / 452 / 459 / 437 ms; no eviction. **Ears (vendor → broker) PASS**: 3 holes / 1.0 s over 19 minutes (run 97: 18 / 9.4 s), (broker → page) 4 arrival holes. `bargein` PASS ×3 (cut-in +7.0–7.2 s → `interrupted` +9.7–10.0 s), PARTIAL in pass 2 on **6.4 s of sound after the `interrupted`** that Yui did not make (she said 「はい。」, 0.5 s — the same unexplained sound as run 90 pass 1; the admitter's microphone remains the suspect). `greet` PASS at T0−12.8 s, vendor transcript PASS 21 / 59, no name echo; muffled f99 2219–2344 Hz on three replies. **The one number left is the greeting's first token, 3457 ms**: the session's warm took 5.2 s and the pre-flight 7.7 s — after a quiet quarter of an hour the local model runs at 150–550 ms a token until its weights are paged back in (the host has 16 GB, Docker Desktop's VM is allotted 12 GB, swap sat at 6.2 of 7 GB). The agent now touches the model every 45 s while idle (dc69ee3; the first tick after the restart took 5.5 s for 10 tokens, the next 23–36 ms). Host disk 12–13 GiB. Reports in `rcai-auto-1788627781415`; `auto-run98.log`, agent `agent-run91-98.log` |
| 97 | **One admission, four passes — 8 / 8 · 7 / 7 · 6 / 7 · 7 / 7; the two run-96 fixes heard in the room** (run 90's launch line, worker restarted before the knock — 7 Xvfb · 14 Chromium left from run 96; 01:14–01:46 JST, 06 Sep; Yui admitted in 30 s, the Tester **715 s** later). `greet` **PASS** with the greeting at **T0−675.8 s** — the admission-long look-back (a382cb7) judged what the admitter heard. `bargein` PASS ×3 (cut-in +7.1–9.2 s → `interrupted` +9.8–12.4 s), PARTIAL by timing in pass 3 (a 7.8 s reply had ended 4.9 s before the cut-in's transcript arrived — nothing to stop, the yield 「はい。」 followed), no 「と思う？」 mis-hear this time so that rule is still unheard in a room. Reply path: **first token 283–467 ms on every turn but one**, three folds (8.4 / 8.0 / 7.8 s) each re-warmed (2.4 / 3.3 / 1.5 s) before the next turn, post-barge-in turns 432 / 467 / 428 ms, no eviction. The one: pass 1 `ask1`, the first cue after the Tester's admission — **first token 10 207 ms on a cache hit** (87 tokens at 117 ms a token against 2; rescore 3.6 s beside it) while the bot host was still joining and recording and the whisper backlog from the chat cue ran on the same GPU; the Tester heard 1.0 s of the answer inside the window. The harness now waits 20 s after both bots are in before the first cue (`SETTLE_MS`, the harness commit that carries this row). Vendor transcript PASS 22 / 61 (Meet's captions gave Yui 「ダイエット！」 and 「うーん！」 for her fillers). Supertonic 1.0–1.3 s ×4; muffled f99 1188–2313 Hz on six replies; ears (vendor → broker) DEGRADED 18 holes / 9.4 s over 28 minutes (broker → page) 47 arrival holes; bot host CPU ≤ 269 %. Host disk 13 → 12 GiB. Reports in `rcai-auto-1788624843950`; `auto-run97.log`, agent `agent-run91.log` |
| 96 | **One admission, four passes — 6 / 8 · 7 / 7 · 7 / 7 · 7 / 7; the reply path clean, two scoring / detector findings** (run 90's launch line, llama-server `-c 16384 --no-clear-idle`; 00:27–00:50 JST, 06 Sep; admitted in **144 s** — Yui let in 118 s before the Tester; worker had 4 stale Xvfb, 0 Chromium). **Every first token 334–454 ms** after the first turn (1192 ms, the first prompt after the greeting), three folds (5.5 / 9.2 / 10.7 s) each re-warmed (2.1 / 1.5 / 1.5 s) before the next turn, post-barge-in turns 435 / 403 / 440 ms, no eviction in the llama log, `bargein` PASS ×3 (cut-in +7.1–7.3 s → `interrupted` +9.5–10.3 s), vendor transcript FAIL 0 / 0 (Yui's transcriber `failed` again, the Tester's produced nothing this time). Pass 1's two FAILs are not the character's: (1) `greet` — she greeted at **T0−117.9 s** (「こんにちは、Yuiです。…」, 5.76 s of audio, the page's `greeting` / `speaking` / `spoke` events all there) and the harness looked back 60 s → `greeting=none`; the look-back is now the admission wait (a382cb7). (2) `bargein` — 「ゆい、これはどう思う？」 reached the recogniser as 「唯イ、これはと思う？」 (「どう」 in a 0.3 s hole) and the trailing 「と思う」 matched the third-person rule (`と思(う|って…)`: narration about her), so `detect` said `name mentioned in third person`; the rescore 「ゆいこれはと思う?」 fell the same way and no turn was taken — 「と思う」 followed by a question mark is now a question to her (`AddressDetector`, 1 test; a382cb7). Passes 2–4: 7 / 7 each, `ask1` / `followup` / `ask2` in full, 「3ページ目」 heard right twice, 「3定理目」 once. Supertonic 1.0–1.1 s ×4, otherwise ≤ 700 ms; muffled f99 1781–2031 Hz on two replies; ears (vendor → broker) DEGRADED 13 holes / 4.0 s, (broker → page) 33 arrival holes; bot host CPU stayed under 250 % (no other build on the host this time). Host disk 13 GiB free throughout. Whisper rescore 11.4 / 9.4 / 6.9 s on the three chat fragments at 15:30:43 Z (queued behind the Tester's join) — cosmetic, the chat cue takes no turn. Reports in `rcai-auto-1788622051754`; `auto-run96.log`, agent `agent-run91.log` |
| 95 | **One admission, four passes — 8 / 8 · 7 / 7 · 7 / 7 · 5 / 7; `--no-clear-idle` heard in the room** (run 90's launch line, llama-server `-c 16384 --no-clear-idle` + warm-up; 23:49–00:10 JST, 05→06 Sep; admitted in **18 s**). Greeting 9.8 s (first token 319 ms), `bargein` PASS ×4 (cut-in +7.0–7.6 s → `interrupted` +9.7–10.8 s), vendor transcript PASS 20 / 60, no name echo. **The eviction is gone**: no `saving idle slot` / `failed to load prompt` in the llama log, the folds ran on their own slot (713 / 950 / 1064 tokens), and the yield turns after a barge-in read 401 / 436 / 410 ms (5.2 / 4.9 s in run 94) — except pass 2's at **2294 ms**, where the fold had *finished* 5 s before the barge-in and the re-warm had not started: the yield turn paid the notes change itself (1320 tokens from the notes on, sim 0.454). That is the fold's own miss, one per fold, landing on whichever turn comes before the warm (three folds, one hit). One more slow turn: pass 4's `bargein` reply first token **3907 ms on a cache hit** (201 tokens re-read at 19 ms/token, 52 tok/s against the usual 480–540) — the host was starved, not the cache: the bot host sat at **477–556 % CPU** from 32 to 77 s into pass 4 and the ears opened 12 holes / 8.7 s there, so `ask1` reached the recogniser as 「今日の予定を教えて。」 with the name gone (no address → no turn) and `followup` had nothing to follow — pass 4's two FAILs are `DEGRADED_BY_HOST_CPU`, the character never heard its name. Passes 1–3: 6 / 1 / 4 holes. Supertonic 1.0–1.3 s ×4 under the same load; muffled f99 1563–2469 Hz on seven replies. Host disk 13 → 6.9 GiB free across the run with none of the project's directories growing (Docker.raw 21 GB throughout) — unexplained, and below the line where run 93 died. Reports in `rcai-auto-1788619735367`; `auto-run95.log`, agent `agent-run91.log` |
...
what was heard, as it must. Heard in run 95 (row 95): no eviction in four
passes, the post-barge-in turns at 401–436 ms, and the one that was not (2.3 s) had met the fold's own
notes change before the re-warm — the miss that is left is one per fold, on whichever turn arrives
between the fold's end and the warm's. The two FAILs of pass 4 were the bot host at 500 % CPU taking
the name out of 「ゆい、今日の予定を教えて」 before it reached the recogniser.

Run 96 (row 96) is the reply path as it now stands, four passes without a slow turn: first token
334–454 ms on every turn after the first, three folds each re-warmed before the next question, no
eviction, post-barge-in turns at 403–440 ms. What failed in pass 1 was the scoring (a greeting 118 s
before the Tester's admission, outside a 60 s look-back) and the address detector (「これはと思う？」
read as narration because 「と思う」 was on the third-person list without regard to the question mark)
— both fixed and covered by tests (a382cb7). Run 97 (row 97) heard the first: Yui was admitted 715 s
before the Tester and the greeting at T0−676 s scored PASS. The 「と思う？」 rule has had no occasion yet.
Run 97's one slow turn was the first cue after the Tester's admission — a cache-hit prompt at 117 ms a
token while the bot host joined and recorded and whisper worked off the chat cue on the same GPU — so
the harness now lets the host settle for 20 s before T0. Every other turn: 283–467 ms.

Run 98 (row 98) is the reply path with everything above in place: first token 253–470 ms on every
turn of four passes, the settle removing run 97's 10 s, no phrase of the voice over a second, the ears'
best run (3 holes / 1.0 s). What is left on the path is the greeting's first token after a long
admission wait — 3.5 s, the local model paged out of a 16 GB host that lends 12 GB to the bot-host VM
(swap 6.2 of 7 GB). The agent now keeps the weights warm with a one-token request every 45 s
(dc69ee3): the first tick after a cold start cost 5.5 s, every tick since 23–36 ms. The measure the
host itself needs is the VM's memory allotment — 12 GB on a 16 GB machine is the test rig's choice,
not the product's — and the sound after a barge-in that Yui does not make (runs 90 and 98) still
wants a microphone check in the room.

Run 99 (row 99): every cue PASS, the greeting's first token 561 ms with the keep-warm, no stray sound
after the barge-ins. The one slow turn was the fold's last coupling with a barge-in — a fold that
finished just after the cut-off, with no time to re-warm before the yield turn — and `interrupt()` now
cancels it (6d0bcd8; not yet heard in a room). The host, meanwhile, is out of headroom: swap 9.1 of
10 GB and the swap files taking the disk the runs need. The bot-host VM's 12 GB is the setting to change.

Run 100 (row 100) closed the fold's last coupling with a barge-in — 399–447 ms on every yield turn, a
fold cancelled by an interrupt and taken after the next reply — and every first token of four passes
sat between 279 and 627 ms. What the run measured instead was the room: a microphone open for the
whole admission (one silent chunk in 122 717), which cut the greeting off, buried one question, and fed
whisper the fragments it makes names from; the name it made (リノース) reached a reply, and the guard
now drops a katakana word the first pass never heard when the rest of the sentence agrees (b0bee3b).
`DEGRADED_BY_OPEN_MIC` is the row's verdict, as in run 71: the character's path did what it should.

Run 101 (row 101), microphone muted: 7 / 8 then 7 / 7 ×3, every first token 210–466 ms. Two things it
settled: the ≈ 6.2 s of sound after one barge-in per admission is not the admitter's microphone (it
recurred with the microphone closed) and belongs to the Tester's capture path; and the katakana guard
was wrong — it dropped whisper's correct 「3ページ目」 for the streaming recogniser's 「3定理目」 four
times and the character repeated the garble. Reverted; the name hallucination is open again, with a
...
Run 102 (row 102) on the 8 GiB VM: the character's cleanest sound yet — no phrase over a second, the ears
PASS, every first token 210–450 ms — and every `bargein` PARTIAL, because the padded exclusion had
removed the quiet the tail scan needed while the honest measure (the character's own audible 1.5–5 s
after the cut-in) read 0.0 s throughout. The verdict now rests on that measure (33bd754). The host still
swaps with the smaller VM; whatever else the machine runs is outside the rig.

Run 103 (row 103) on the corrected verdict: 8 / 8 · 7 / 7, then two PARTIALs by timing alone. The
character's path is where it has been since run 98 — every first token under half a second, the voice
under a second a phrase, the ears clean. Two things to carry: the answer after a barge-in echoes the
cut-in's request rather than the question (a prompt matter, not latency), and a knock needs ≈ 15 GiB
of host disk because the bot host holds ≈ 13 GB of transient files while the bots are in the room.

Run 104 (row 104) was void: the Attendee containers rebuilt after the prune joined, loaded the page,
played the Tester's cues — and captured no meeting audio in either bot, so nothing reached the broker
and the character sat deaf for two passes. Run 105's pre-check (d335e93) left within 30 s, and the
cause is now known (row 105): the rebuild had used Attendee's plain compose file, not the project's
overlay, so the worker lacked `--disable-features=LocalNetworkAccessChecks` and Chromium 151 blocked
the page's media socket to the worker. The stack is recreated through `attendee-selfhost.sh up` again;
the next knock is the confirmation.

Run 106 (row 106) confirmed it: the relay carried audio within the pre-check, the character answered,
and Meet's captions heard her name as ゆい for the first time. The run's FAILs were the room's — an
open microphone with a television behind it, which the character dutifully answered — and the host's,
swapping 13.8 GB with other sessions on the machine. The rig's own path stood: first token 247–584 ms
on every turn.

Run 107 (row 107), same room, same open microphone: 28 of 29 cues, every barge-in stopped, the one
loss a question whose name fell into a hole while she was answering the room. The character's path is
where it was in runs 98–103; what varies from run to run now is the room and the host.

Run 109 (row 109): 27 of 29 again, the room open again, and the one FAIL a recogniser hole that
turned the third-person cue into a nameless follow-up the character answered before the rescore could
say otherwise. That is the gate's next item on the character's side: hold a follow-up that opens with a
fragment until the second opinion is in.

### A general user's questions, and what they showed (2026-09-07, converse)

Asked through the spoken-conversation gate (`pnpm reality:converse`, macOS `say` in, the character's
voice out, re-read by the same recogniser): 「今日のニュースを教えて」「今日の東京の天気はどう？」「今、
何時ですか？」「十二かける八はいくつ？」「自己紹介をして」「Live2Dって何？」「おすすめの映画を一本」「明日
までにやることを整理したい」. The answers: **weather invented** (「東京は今日は晴れで…」), **the time invented**
(「およそ午後3時ですよ」 at two in the morning), meeting facts invented for the general questions (「新しい顧客
向けサービスの立ち上げに関する会議です」), and the news question drew no reply at all. Two fixes (77e4b9d):
the meeting instructions now say real-time information (news, weather, prices, traffic) and search are not
available — say so briefly and offer one point from the room — and the turn prompt carries 【現在時刻】 from
the page's clock (sim 74: `【現在時刻】2026-09-07 09:34` in the turn), so the one live fact she can state
is right; the free-talk persona gets the same rule. Not yet heard in a room.

The same check showed every utterance losing its first ~1.4 s (「十二かける八はいくつ」 → 「×8はいくつ?」,
「ゆいさんの自己紹介をして」 → 「ご紹介をして」). Chased with the agent's utterance dumps: **the audio is
whole** (the dumped clip carries the whole phrase, envelope matched against the clip sent), and both
recognisers still read only the tail — SenseVoice 「ゆさん、今日のニュースを教えて」, whisper 「今日の
ニュースを教えて」 (14.6 s, cold and loaded). The synthetic voice's first syllables, not the pipeline;
in the room the same words are lost to holes, which is a different thing. No code change.

Two more from the same afternoon (47988e7): the Attendee streamer captured the character's page at
**16 kHz** and sent that to the meeting — the telephone f99 of 1.6–2.5 kHz on most replies since run 89;
it captures and sends at 48 kHz now (Opus fullband; streamer restarted). And the second recogniser no
longer re-reads every line of room chatter — only short lines, questions and requests, name-sized
tokens and fragment starts — which is where a better reading can change anything; the rest was GPU
time taken from the voice (run 106: ~20 rescores a pass on lines nobody addressed to her).

Run 113 (row 113) heard the 48 kHz voice: f99 4–6.7 kHz on the clean replies, the first time above
2.5 kHz in the room. Everything else in the run was the host (143 ears holes, whisper at 8–15 s, the
voice over a second twenty times) and a microphone left open (0.05 % silence). The gate's next number
to move is not in the character: it is the machine she runs on.

Run 114 (row 114) put a number on that: host load average **50** during the passes, from macOS's own
background work (`mobileassetd`, `ANECompilerService`, `modelcatalogd`, `mediaanalysisd`) rather than
from the rig, 254 s of ears holes in fourteen minutes, and the vendor's API timing out inside a pass.
The character answered in 453–859 ms whenever a question reached her. A gate run on this machine is
now a measurement of what else the machine is doing; the fix is a bot host that is not this laptop, or
a window when macOS is quiet.

Run 115 (row 115) had the quieter machine (load 5–11) and confirmed the voice: f99 median 3 875 Hz,
up to 7 156. It also had the microphone open from the first cue to the last — 0.08 % silence in the
mix — and that, not the host, took its four `bargein` PARTIALs and two lost questions. Across runs
113–115 the two remaining levers are the room's microphone and this machine's other work; the
character's own numbers (first token 238–859 ms, every question answered where it arrived, the
demonstrative check, the name in the captions) have not moved since run 110.

Picture: 720p was tried in run 116 (row 116) and reverted the same hour. Four times the pixels through
software GL and Python VP8 doubled the ears' holes (387 / 239 s against 182 / 111 s) and cost the
character three questions a pass; the page itself rendered at 53 fps and the voice kept its band, so
the limit is the emulated encoder on this host, not the avatar. 640x360 stands until the bot host has
a hardware encoder.

Run 110 (row 110), microphone muted: not one FAIL in four passes; the two PARTIALs were replies that
ended before the cut-in arrived. First token 220–639 ms over 21 turns, the greeting at 220 ms. With
the room quiet the character's path is the one measured in runs 98–103; the two open items on her side
are the fragment-opening follow-up (run 109) and the barge-in's request colouring the next answer.

Run 111 (row 111), microphone muted again: two clean passes, then the ears gave out — 52 s of holes in
