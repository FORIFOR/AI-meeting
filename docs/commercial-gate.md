...
and the character sat deaf for two passes. The cause is not identified (same compose, image, source,
env and mounts as the containers that ran run 103 an hour earlier). The stack was recreated whole; the
next knock is a probe, and the harness now leaves within 30 s if the room is inaudible (d335e93).
