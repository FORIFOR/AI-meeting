...
import { AddressDetector } from '../../../../packages/meeting-core/src/addressDetector.js';
...
const folder='/tmp/auto-go-address-audio';mkdirSync(folder,{recursive:true});
const sleep=(ms:number)=>new Promise(r=>setTimeout(r,ms));const rows:any[]=[];
const detector=new AddressDetector({names:['Yui','ゆい','ユイ','結衣','唯'],soundalikes:['い','うい','つい','ゆ']});
for(const voice of ['greet','numbers','followup','long','resume']){
 execFileSync('ffmpeg',['-y','-loglevel','error','-i','/tmp/hosted-tester-'+voice+'.mp3','-ar','16000','-ac','1','-f','s16le',folder+'/'+voice+'.pcm']);
 const input=readFileSync(folder+'/'+voice+'.pcm');
 ...
 const provider=new GeminiLiveProvider({brokerUrl:'https://ai-meeting-broker-pdygkns5gq-an.a.run.app',wsFactory:u=>new WS(u),googleSearch:false});
 provider.onEvent(e=>{
 ...
 row.addressDetections=row.heard.map((text:string)=>({text,detection:detector.detect(text)}));
 ...
 writeFileSync(new URL('./address-audio.json',import.meta.url),JSON.stringify({scope:'Actual synthetic Meet test clips through current Gemini provider and AddressDetector; not the Hosted relay or full meeting controller',rows},null,2)+'\n');
}
