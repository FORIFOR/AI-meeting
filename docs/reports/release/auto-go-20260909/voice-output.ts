...
const folder='/tmp/long-utterance-voice-samples';mkdirSync(folder,{recursive:true});
...
for(const voice of ['Kore','Aoede','Leda','Puck','Zephyr']){
 ...
 try{
  await provider.connect({voice,systemPrompt:'あなたはYuiです。質問に日本語で短く正確に答えてください。',language:'ja-JP',mode:'free_talk',privacyMode:'default'});
 ...
 writeFileSync(new URL('./voice-output.json',import.meta.url),JSON.stringify({scope:'Real cloud provider output before physical playback; 1 synthetic utterance per voice; no human quality score',rows},null,2)+'\n');
}
