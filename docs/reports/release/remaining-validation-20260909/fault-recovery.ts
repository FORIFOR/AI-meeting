...
let offlineUntil = 0;
const faultKind = process.env.FAULT_KIND ?? "network";
...
const provider = new GeminiLiveProvider({
  brokerUrl,
  fetchImpl: async (url, init) => { if(Date.now()<offlineUntil) {
    if (faultKind === "rateLimit") return new Response(JSON.stringify({error:"Injected rate limit"}), {status:429,headers:{"content-type":"application/json","retry-after":"1"}});
    if (faultKind === "timeout") throw new DOMException("Injected request timeout", "TimeoutError");
    throw new Error("Injected network offline");
...
try {
  ...
  offlineUntil=faultStartedAt+10000;
  sockets.at(-1)!.terminate();
  phase = "network reconnect";
  await waitFor(() => readyCount === 2);
  observations.recoveryElapsedMs=Date.now()-faultStartedAt;
  ...
  const report = { schemaVersion: 1, measuredAt: new Date().toISOString(), scope: `Real Gemini TCP disconnected; ${faultKind} injected at token fetch for 10 seconds; not physical network or device testing`, faultKind, status, phase, observations, rows };
