  ...
  return (
    <div className="pick">
      ...
      <div className="pick__stage">
        <div key={current.id} className={`pick__portrait ${current.id === p.settings.characterId ? "pick__portrait--accent" : ""}`}>
          <span className="pick__glyph">{PLATE[current.id] ?? current.name.slice(0, 1)}</span>
        </div>
      </div>
