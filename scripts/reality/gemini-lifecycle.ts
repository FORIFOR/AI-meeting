...
const waitFor = async (predicate: () => boolean, ms = 20_000) => {
  ...
  if (!predicate()) throw new Error("observation timed out");
};
...
const provider = new GeminiLiveProvider({
  ...
  wsFactory: (url) => {
    ...
    socket.addEventListener("message", async (event) => {
      ...
      const update = JSON.parse(text).sessionResumptionUpdate;
      if (update?.resumable && update.newHandle) serverHandles.add(update.newHandle);
    });
    return socket as unknown as WebSocketLike;
  },
});
provider.onEvent((event) => { eventCount++; if (event.type === "session_ready") readyCount++; });
...
try {
  ...
  await provider.updateContext(config);
  await provider.sendText("短く『準備できました』と返してください。");
  await waitFor(() => serverHandles.size > 0, 20_000);
  sockets.at(-1)!.close(4000, "release gate network interruption");
  ...
  const report = { schemaVersion: 1, measuredAt: new Date().toISOString(), scope: "real Gemini connection lifecycle only; no voice, platform or human gate claim", status, rows };
