...
const required = ["PROJECT_ID", "MEETING_TOKEN_SECRET", "GEMINI_API_KEY"];
...
if (process.env.WEB_PUBLIC_URL) envVars.push(`RECALL_BOT_PAGE_URL=${process.env.WEB_PUBLIC_URL}/agent/runtime`);
const deployArgs = ["run", "deploy", service, "--project", project, "--region", region, "--image", image, "--platform", "managed", "--allow-unauthenticated", "--port", "8787", "--min", "0", "--max", process.env.MAX_INSTANCES ?? "1", "--set-env-vars", envVars.join(",")];
