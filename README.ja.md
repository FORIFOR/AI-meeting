...
音声アシスタントが文字起こしで終わらないように、AI Meetingは変更案を確認するまで保留し、確定した決定だけをブラウザー内のタスクとして保存します。次に開いたときも続きから使える、音声からタスクへ変える流れが中心です。手元で動かし、実装を読み、用途に合わせて調整できます。
...
**ベータ版です。** 2026年9月14日に自動テスト1,100件、型検査、OSSビルド監査が合格しました。従来のVRM制約サンプルでは、日本語50文、割り込み20回、60分連続稼働を確認しています。新しいAvatarSample_Bはブラウザー上と45秒の実Gemini録画で確認しました。このモデル単体での60分連続検証は未実施です。
