...
const DONE_NEGATION = /(?:[?？]|ません|ない|なかった|まだ|未(?:完了|対応|実施|送信|提出)|して(?:い|お)ない|できていない|終わっていない|完了していない|\bnot(?:yet)?\b|haven['’]?t|hasn['’]?t|didn['’]?t|isn['’]?t|aren['’]?t)/iu;
const DONE_HYPOTHETICAL = /(?:ったら|えたら|けたら|したら|できたら|なら|場合|予定|つもり|したい|しよう|しておく|\bwhen\b|\bif\b|\bonce\b|plan(?:ning)?to|\bwill\b|goingto|wantto)/iu;
const DONE_EVIDENCE = /(?:完了(?:した|しました|済み|です)?|終わった|終わりました|済ませた|済ませました|やった|やりました|対応済み|送った|送りました|提出した|提出しました|買いました|買った|\bdone\b|\bcompleted\b|\bfinished\b|\bsent\b|\bsubmitted\b)/iu;
...
function validateStatusEvidence(status: ConversationTask['status'], quote: string): string | undefined {
  ...
  if (status === 'deferred' && (DONE_NEGATION.test(text) || !DEFER_EVIDENCE.test(text))) return 'deferred status lacks explicit deferral evidence';
  return undefined;
}
export class TaskLedger {
  ...
  propose(args: Record<string,unknown>): { proposal?: TaskProposal; error?: string } {
    ...
    const result=preview.applyValidated(args,source,true);
  ...
  resolve(id:string,accept:boolean): {tasks:ConversationTask[];error?:string} {
    ...
    return this.applyValidated(p.args,p.source,true);
  }
  ...
  apply(args: Record<string, unknown>, latestUserText: string): { tasks: ConversationTask[]; error?: string } {
    return this.applyValidated(args, latestUserText, false);
  }
  /** UI approval bypasses the conservative language heuristic, never structure/quote/ID checks. */
  private applyValidated(args: Record<string, unknown>, latestUserText: string, reviewed: boolean): { tasks: ConversationTask[]; error?: string } {
    ...
    for(const raw of args.operations) {
      ...
      if(!reviewed && op.status!==undefined){const evidenceError=validateStatusEvidence(status as ConversationTask['status'],latestUserText);if(evidenceError)return fail(evidenceError);}
      if(op.action==='add') {
