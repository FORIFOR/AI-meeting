...
it('rejects ID overwrite, invalid time and verification without evidence',()=>{
 const ledger=new ExecutionLedger();ledger.create({id:'a',idempotencyKey:'a',action:'a',payload:{},now:1});
 expect(()=>ledger.create({id:'a',idempotencyKey:'b',action:'a',payload:{}})).toThrow();
 expect(()=>ledger.create({id:'b',idempotencyKey:'b',action:'a',payload:{},now:NaN})).toThrow();
 ledger.transition('a','approved',{now:2});expect(()=>ledger.transition('a','executing',{now:1})).toThrow();
 ledger.transition('a','executing',{now:3});expect(()=>ledger.transition('a','verified')).toThrow();ledger.transition('a','unknown',{now:4});expect(()=>ledger.transition('a','executing',{now:5})).toThrow();
});
